...
// Every purchase-affecting route checks the backend clock,
// not the browser countdown, so the sales window cannot
...
async function requireSalesOpen(req, res, next) {
  try {
    ...
    if (
      !settings ||
      new Date() > new Date(settings.sales_close_at) ||
      settings.status === "closed"
    ) {
      return res.status(410).json({
        error: "Sales are closed.",
      });
    }
    ...
    res.status(500).json({
      error: "Could not check sales status.",
    });
  }
}
  ...
  try {
    ...
    const typesResult = await pool.query(
      `SELECT
         ...
         name,
         price,
         (total_quantity - sold_quantity - reserved_quantity) AS available
       FROM ticket_types`
    );

    res.json({
      settings: settingsResult.rows[0] || null,
      ticketTypes: typesResult.rows,
    });
  } catch (err) {
...
router.post("/orders", requireSalesOpen, async (req, res) => {
  ...
  try {
    ...
    const available =
      Number(type.total_quantity) -
      Number(type.sold_quantity) -
      Number(type.reserved_quantity);
    ...
    const total =
      Number(type.price) * Number(quantity);
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(502).json({
        error:
          "STK push failed. Try again or use another payment method.",
      });
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    ...
    res.json({
      paymentId,
      message:
        "Wire this to your card processor's checkout session.",
    });
  }
);
...
router.post(
  ...
  async (req, res) => {
    ...
    res.json({
      ...
      amount: order.total_amount,
      note:
        "Payment is matched manually or via your aggregator's PesaLink webhook.",
    });
  }
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const pendingMpesa =
        pendingMpesaResult.rows[0];

      const pendingBank =
        pendingBankResult.rows[0];
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        "POST /admin/close-sales failed:",
        err
      );

      res.status(500).json({
        error: "Could not close sales.",
      });
    }
  }
);
